...
As of early 2021, MSG can be run in a Docker container generated by this currently private Janelia repo: https://github.com/JaneliaSciComp/msg-docker
...
* `default_container_options`: (string, Default='') Cluster job submission option if running in a container. This option is inserted before the actual executable command in cluster submissions so the child jobs will also run in the container. For Singularity, it would look something like `singularity exec <container location>`
